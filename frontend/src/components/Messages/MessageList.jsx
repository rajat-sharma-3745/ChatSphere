...
const MessageList = ({
  ...
  useEffect(() => {
   if (!targetMessageId.current ) return;
  
    let el = document.getElementById(targetMessageId.current);
    if (el) {
  ...
  return (
    <div ref={ref} className="flex-1 min-h-0 overflow-y-auto px-4 py-4 bg-gray-50">
      {loading ? (
        <ChatSkeleton />
      ) : messages.length > 0 ? (
        messages.map((msg) => {
